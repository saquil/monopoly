...
public class Carte {
	...
	int effetArgent;
	/**
	 * La position sur laquelle le joueur sera amené (-1 si la carte ne deplacer
	 * pas le joueur)
	 */
	int effetPosition;
	...
	/**
	 * Applique les effets de la carte à un joueur
	 * @param unJoueur Le joueur qui sera affecté par la carte
	...
	public String affecterJoueur(Joueur unJoueur) {
		if (this.getEffetPosition() >= 0) {
			...
			if (this.getEffetPosition() == 10) {
				...
				return "Le joueur " + unJoueur.getId() + " a été emprisonné";
			}
		}
